...
with open("cognitive-protocol.md") as f:
    constraint_catalyst_rules = f.read()

model = genai.GenerativeModel(
    model_name="gemini-2.0-flash",
    system_instruction=constraint_catalyst_rules
)
```
...
Place First Principles first in the system instruction, then Constraint as Catalyst. Both are concise enough to fit together without significant context cost.
